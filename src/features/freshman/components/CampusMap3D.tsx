...
export function CampusMap3D({ compact = false }: { compact?: boolean }) {
  ...
  useEffect(() => {
    ...
    loader.load(
      `${import.meta.env.BASE_URL}Universidad.glb`,
      (gltf: any) => {
        const model = gltf.scene;

        // Traverse and adjust shadows
    ...
    const animate = () => {
      ...
      if (cameraRef.current && controlsRef.current) {
        ...
        if (lerpTargetLookAt.current && lerpTargetPos.current) {
          // Temporarily disable controls to prevent damping from fighting our manual lerp
          ctrl.enabled = false;
          
          // Smoothly interpolate control target (look-at)
          ctrl.target.lerp(lerpTargetLookAt.current, 0.08);
          // Smoothly interpolate camera position
          cam.position.lerp(lerpTargetPos.current, 0.08);

          // If close enough, snap to target and finish
          if (ctrl.target.distanceTo(lerpTargetLookAt.current) < 0.05) {
            ctrl.target.copy(lerpTargetLookAt.current);
            cam.position.copy(lerpTargetPos.current);
            lerpTargetLookAt.current = null;
            lerpTargetPos.current = null;
            ctrl.enabled = true; // Re-enable user controls
          }
        }
      }
    ...
    <div
      ...
      className={`relative w-full overflow-hidden border border-gray-800 bg-gray-955 shadow-2xl flex select-none ${
        compact 
          ...
          : 'h-[70dvh] md:h-[70vh] rounded-3xl flex-col md:flex-row'
      }`}
    >
      {/* 3D View Container */}
      <div
          ...
          {mapMode === '2d' && (
            ...
            >
              ...
              <div className="w-full h-[82%] rounded-2xl border border-gray-800 bg-gray-950 overflow-auto flex items-center justify-center p-4 cursor-zoom-in pointer-events-auto scrollbar-thin scrollbar-thumb-gray-850">
                <img
                  src="https://i.ibb.co/vfZh5GJ/Gemini-Generated-Image-dcsxbadcsxbadcsx.png"
