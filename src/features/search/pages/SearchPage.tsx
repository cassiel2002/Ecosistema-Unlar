...
export function SearchPage() {
  ...
  // Sync URL changes to local state (for when searching from the header while already on this page)
  useEffect(() => {
    const q = searchParams.get('q') || '';
    if (q !== query && q !== initialQuery) {
      setQuery(q);
    }
  }, [searchParams, setQuery]);

  // Update URL when query changes (optional, but good for shareable links)
  useEffect(() => {
    if (query) {
